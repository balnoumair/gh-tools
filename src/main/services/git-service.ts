...
export async function selectRepo(): Promise<GitRepo | null> {
  ...
  if (!fs.existsSync(gitDir)) {
    // Could be a worktree — check via simple-git
...
export async function checkoutBranch(
  ...
  try {
    const git = getGit(repoPath);
    // If it's a remote branch like "origin/feature", checkout as local tracking branch
...
export async function createBranch(
  ...
  try {
    ...
    await git.checkoutLocalBranch(args[0]);
    if (startPoint) {
