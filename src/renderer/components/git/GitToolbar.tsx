...
function ToolbarButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      title={label}
      className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-ghv-text-dim hover:text-ghv-text hover:bg-ghv-surface-hover rounded transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
    >
      {children}
      <span>{label}</span>
    </button>
...
export default function GitToolbar() {
  ...
  return (
    <div className="h-9 bg-ghv-surface/50 border-b border-ghv-border flex items-center px-2 gap-1 shrink-0">
      ...
      <div className="w-px h-4 bg-ghv-border mx-1" />
